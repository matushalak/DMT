...
def train_and_evaluate(
    ...
    if save_fig:
        ...
        plt.plot(range(1, len(val_accuracies)+1), val_accuracies, label='Eval Accuracy', marker='o')
...
if __name__ == "__main__":
    ...
    # select 50 most correlated features
    ...
    n_top_features = 50
    top_features = correlation_with_CATtarget.drop(['target'], errors='ignore')
    top_features = top_features.abs().sort_values(ascending=False).index[:n_top_features].tolist()

    top_features += ["date", "time_of_day_non_encoded", "id_num"]
    ...
    if do_hyperparameter_tuning:
        ...
        param_grid = {
            # Sequence parameters
            "seq_length": [5, 10, 20],                 # Test shorter and longer sequence windows
            
            # Model architecture parameters
            "model_type": ["GRU", "SimpleRNN", "LSTM"],            # Focus on the two stronger model types
            "hidden_dim": [16, 32, 64],             # Test different capacities
            "num_layers": [1, 2],                     # More than 2 layers rarely helps for this task
            "dropout": [0.2, 0.3],               # Test different regularization strengths
            
            # Training parameters
            "batch_size": [256],                       # Keep batch size constant
            "learning_rate": [0.001, 0.0001],  # Test around the typical Adam default
            ...
            "transform_target": [False, True],               # Always normalize target
            ...
            "patientce": [15],                       # Early stopping patience
        }
